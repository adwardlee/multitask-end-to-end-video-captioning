...
class Video_Caption_Generator():
    ...
    def build_model(self):
        ...
        ########### attibute  ###############3
        attribute_video = tf.reshape(attribute_net, [self.batch_size, self.n_video_lstm_step, conv_dim])####################################################  add  #####
        #####################################################
        ...
        #####################  attribute ############
        fusion_feature = tf.concat([image_emb, attribute_video],axis=2)####################################################  add  #####
        ...
        ######## multilabel loss########
        attribute_feature = tf.reduce_mean(attribute_video, axis=1)####################################################  change  #####
...
model_path = './fix_multitask_models'
...
lstm_model = '/home/llj/tensorflow_s2vt/fix_models/batch_size16_fix_models_attribute_fc_1stlstm_1e-4_concat_dim1000-14'
...
out_file = 'fix_multitask_models/multitask_concat_1stlstm_1e-6_dim1000_val.txt'
model_name = 'multitask_concat_1stlstm_1e-6_dim1000'
...
def train():  ###### move caption (input_sentence) one column left and also need to move caption_mask (cont_sent)one column left ########################################################llj
   ...
   # summary_op = tf.summary.merge_all()
    ...
    with open(out_file, 'a') as f:
        ...
        harmmean = 2.0 / (count_pos / float(true_positive+ 0.00001) + count_neg / float(true_negative+0.00001)) if ((count_pos + count_neg) > 0) else 0
       ...
       # train_captions, video_frames, train_vid = get_video_feature_caption_pair(video_train_sent_file, video_path,num_frame_per_video=n_video_lstm_step)
        ...
        for start, end in zip(range(0, len(index) - batch_size, batch_size), range(batch_size, len(index), batch_size)):
            ...
            if np.mod(iter_count, 2000) == 0:
                ...
                plt_save_dir = "./fix_multitask_loss_imgs"
                plt_save_img_name = str(iter_count) + 'alpha_' + str(alpha) + 'based_bestreinforce_fcconcat_1stlstm_cider.png'
            ...
            if np.mod(iter_count, 2000) == 0:
                ...
                saver.save(sess, os.path.join(model_path,
                                              str(n_video_lstm_step) + 'batch_size' + str(batch_size) +'alpha_'+ str(alpha)+ '_' +model_name),
        ...
        plt_save_dir = "./fix_multitask_loss_imgs"
        plt_save_img_name = str(iter_count) + 'alpha_' + str(alpha) + 'based_best5frame_fcsum_1stlstm.png'
...
def evaluation(model_path='/home/llj/tensorflow_s2vt/fix_multitask_models/'):
    ...
    with open('fix_multitask_models/multitask_concat_1stlstm_1e-6_alpha0.01_batch4_dim500_test', 'a') as f:
      for i in xrange(16000,64000,46000):
        model_path_last = model_path + '5batch_size4alpha_0.01_multitask_concat_1stlstm_1e-6_dim500-' +str(i)
        saver.restore(sess,model_path_last)
        all_decoded_for_eval = {}
        test_index = list(range(len(test_captions)))
        random.shuffle(test_index)
