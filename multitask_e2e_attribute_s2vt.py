...
class Video_Caption_Generator():
    ...
    def build_model(self):
        ...
        return loss, video_frames, caption, caption_mask, probs, true_labels, multilabel_loss
...
model_path = './new_multitask_models'
...
lstm_model = '/data1/lijun/tensorflow_s2vt/new_s2vt_models/batch_size64_new_s2vt_model-22'
...
out_file = 'new_multitask_models/multitask_scores_onval_best5lstm_model11_1e-6_batch16_alpha0.01.txt'
...
def train():  ###### move caption (input_sentence) one column left and also need to move caption_mask (cont_sent)one column left ########################################################llj
    ...
    tf_loss, tf_video, tf_caption, tf_caption_mask, tf_probs, tf_labels, tf_multilabel_loss = model.build_model()
        ...
        for start, end in zip(range(0, len(index) - batch_size, batch_size), range(batch_size, len(index), batch_size)):
            ...
            if np.mod(iter_count, 2000) == 0:
                ...
                saver.save(sess, os.path.join(model_path,
                                              str(n_video_lstm_step) + 'batch_size' + str(batch_size) +'alpha_'+ str(alpha)+ 'based_best5frame_model'),
