...
def train():  ###### move caption (input_sentence) one column left and also need to move caption_mask (cont_sent)one column left ########################################################llj
    ...
    optimistic_restore(sess,'new_attribute_models/5_lr_1e-05_batch_size16alpha_1attribute_model-14000')
    #optimistic_restore(sess,'new_attribute_models/5batch_size8alpha_0.01attribute_model-8000')
   ...
   # summary_op = tf.summary.merge_all()
    ...
    with open('fix_new_e2e_models/lr1e-3.txt', 'a') as f:
        ...
        for start, end in zip(range(0, len(index) - batch_size, batch_size), range(batch_size, len(index), batch_size)):
            ...
            if np.mod(iter_count, 500) == 0:
                ...
                saver.save(sess, os.path.join(model_path,
                                              str(n_video_lstm_step) + 'batch_size' + str(batch_size) + 'noinit_e2e_batch64_lr1e-3_train_withoutval'),
...
def evaluation(model_path='/home/llj/tensorflow_s2vt/e2e_models/'):
    ...
    with open('fix_e2e_models/batch4_lr1e-5_test.txt', 'w') as f:
