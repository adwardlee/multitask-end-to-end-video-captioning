...
class Video_Caption_Generator():
    ...
    def build_loss(self):###################### change batch_size to 4*batch_size to multisample sentence from multinomial for one video
        video = tf.placeholder(tf.float32, [self.batch_size, self.n_video_lstm_step, self.dim_image])  ###llj
...
model_path = './new_reinforcement_models'

video_train_feature_file = '/data1/lijun/tensorflow_s2vt/5frame_train.txt'

video_test_feature_file = '/data1/lijun/tensorflow_s2vt/5frame_val.txt'
...
model_name = 'reinforce_onesamp_model_another'
out_file = 'reinforcement_models/reinforce_onesamp_model_another'
image_file = 'reinforce_onesamp_model_another'
# =======================================================================================
# Train Parameters
# =======================================================================================
dim_image = 1536
        ...
        for start, end in zip(range(0, len(index)-batch_size, batch_size), range(batch_size, len(index), batch_size)):
            ...
            #for i in xrange(4):
                #for j in xrange(len(features_batch)):
                    #features_batch4.append(features_batch[j])
           ...
           # r = [evaluate_captions([k],[v]) for k,v in zip(ref_decoded, multi_decoded)]
            ...
            ###################
            feed_dict = {loss_masks: mask, loss_captions: samples, loss_features: features_batch, rewards: r,
                         base_line: b}
...
def evaluation(model_path='/home/llj/tensorflow_s2vt/reinforcement_models/'):
    test_captions, test_features = get_video_feature_caption_pair(video_test_sent_file, video_test_feature_file)
    ...
    #model_path_last = '/home/llj/tensorflow_s2vt/reinforcement_models/reinforce_multisample2_model-17'
    ...
    with open('reinforcement_models/reinforce_onesamp_model_test.txt', 'a') as f:
      for i in xrange(8,9):
        model_path_last = model_path + 'reinforce_onesamp_model_another-' + str(i)
        saver.restore(sess, model_path_last)
        all_decoded_for_eval = {}
        test_index = list(range(len(test_captions)))
        random.shuffle(test_index)
