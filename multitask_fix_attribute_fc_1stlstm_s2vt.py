...
feature_model = '/home/llj/tensorflow_s2vt/multilabel_models/batch_size8multilabel_fc3e-1_stopgradient_dim500-68'
...
out_file = 'fix_models/fix_models_attribute_fc_1stlstm_1e-4_concat_another.txt'

model_name = 'fix_models_attribute_fc_1stlstm_1e-4_sum'
...
######################################### caption evaluation ##########################
    ...
    for epoch in range(0, n_epochs):
       ...
       # train_captions, video_frames, train_vid = get_video_feature_caption_pair(video_train_sent_file, video_path,num_frame_per_video=n_video_lstm_step)
        ...
        with open('fix_models/5uniform_attribute_fc_1stlstm_concat_1e-4_another_val.txt', 'a') as f:
            all_decoded_for_eval = {}
            test_index = list(range(len(test_captions)))
            random.shuffle(test_index)
            ref_decoded = {}
        ...
        plt_save_img_name = str(epoch) + '_5uniform_attibute_fc_1stlstm_cider_concat_val.png'
        plt.figure(1)
        ...
        plt_save_dir = "./fix_loss_imgs"
        plt_save_img_name = str(epoch) + '_5uniform_concat_val.png'
        plt.figure(2)
        ...
        if np.mod(epoch, 1) == 0:
            ...
            saver.save(sess, os.path.join(model_path, 'batch_size' + str(batch_size)+ '_fix_attribute_fc_1stlstm_concat_1e-5_model'), global_step=epoch)
            if early_stopping > 8:
                return
            early_stopping += 1
...
def evaluation(model_path='/home/llj/tensorflow_s2vt/fix_models/'):
    ...
    with open('fix_models/fix_models_attribute_fc_1stlstm_1e-4_concat_another_test.txt', 'a') as f:
      for i in xrange(1,13,1):
        model_path_last = model_path + 'batch_size16_fix_attribute_fc_1stlstm_concat_1e-5_model-' +str(i)
        saver.restore(sess,model_path_last)
        all_decoded_for_eval = {}
        test_index = list(range(len(test_captions)))
        random.shuffle(test_index)
