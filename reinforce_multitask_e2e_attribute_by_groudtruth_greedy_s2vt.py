...
class Video_Caption_Generator():
    ...
    def build_loss(self):
        ...
        return loss, video_frames, caption, caption_mask
...
out_file = 'reinforcement_multitask_models/reinforcement_multitask_scores_onval_lambda0_greedy_groundtruth.txt'

save_model_name = 'reinforce_multitask_model_lambda0_greedy_groundtruth'

save_loss_imgs = 'reinforce_multitask_lambda0_greedy_groundtruth'

save_cider_imgs = 'reinforce_multitask_lambda0_greedy_groundtruth'
...
def train():  ###### move caption (input_sentence) one column left and also need to move caption_mask (cont_sent)one column left ########################################################llj
    ...
    model_loss, model_features, model_captions, model_caption_masks, _ = model.build_model()
    ...
    sum_loss = -(1-lambda_loss)*tf.reduce_sum(tf.transpose(tf.multiply(tf.transpose(loss,[2,1,0]), residual),[2,1,0]))/norm + lambda_loss * model_loss### transpose loss:batchxvideo_step_word_dim
...
######################################### caption evaluation ##########################
    ...
    for epoch in range(0, n_epochs):
        ...
        for start, end in zip(range(0, len(index)-batch_size, batch_size), range(batch_size, len(index), batch_size)):
            ...
            r = evaluate_captions_cider(ref_decoded,greedy_decoded)
           ...
           # r = [evaluate_captions([k],[v]) for k,v in zip(ref_decoded, multi_decoded)]
            ...
            feed_dict = {loss_masks: greedy_mask, loss_captions: greedy_words, loss_features: video_batch, rewards: r, base_line: b, model_features: video_batch, model_captions: captions_ind, model_caption_masks: captions_mask}
            ...
            if np.mod(iter_count, iteration_size) == 0:
                ...
                plt_save_dir = "./reinforcement_multitask_loss_imgs"
                plt_save_img_name = str(iter_count) + save_cider_imgs
        ...
        plt_save_dir = "./reinforcement_multitask_loss_imgs"
        plt_save_img_name = str(iter_count) + save_loss_imgs
...
def evaluation(model_path='/home/llj/tensorflow_s2vt/reinforcement_multitask_models/'):
    ...
    with open('reinforcement_multitask_models/training_noval_resmodel_test_false_bn.txt', 'w') as f:
        all_decoded_for_eval = {}
        test_index = list(range(len(test_captions)))
        random.shuffle(test_index)
        ref_decoded = {}
