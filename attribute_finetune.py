...
class Video_Caption_Generator():
    ...
    def build_model(self):
        ...
        loss = weight_decay_loss + multilabel_loss  #### label smoothing
        return loss, video_frames,  true_labels, multilabel_loss
...
model_path = './new_attribute_models'
...
video_test_sent_file = '/data1/lijun/tensorflow_s2vt/msvd_sents_val_lc_nopunc.txt'
...
lstm_model = '/data1/lijun/tensorflow_s2vt/res_models/best_5frames/batch_size16_5uniform_third_model-11'
...
out_file = 'new_attribute_models/attribute_lr1e-5_batch16.txt'
...
alpha = 1 ###### loss weight of multilabel , 1- alpha: loss weight of captioning
...
def get_video_feature_caption_pair_best(sent_file=video_train_sent_file, frame_path=video_path, num_frame_per_video = n_video_lstm_step,prefix=''):
    sents = []
    vid = []
    video_frames = {}
    with open(sent_file, 'r') as video_sent_file:
...
def train():  ###### move caption (input_sentence) one column left and also need to move caption_mask (cont_sent)one column left ########################################################llj
    ...
    if not os.path.exists('./new_vocab1_data/wordtoix') or not os.path.exists('./new_vocab1_data/ixtoword'):
    ...
    tf_loss, tf_video, tf_labels, tf_multilabel_loss = model.build_model()
    ...
    grads_rl, _ = tf.clip_by_global_norm(tf.gradients(tf_loss, tf.trainable_variables(), aggregation_method=tf.AggregationMethod.EXPERIMENTAL_ACCUMULATE_N), 10)
    ...
    for epoch in range(0, n_epochs):
       ...
       # train_captions, video_frames, train_vid = get_video_feature_caption_pair(video_train_sent_file, video_path,num_frame_per_video=n_video_lstm_step)
        ...
        for start, end in zip(range(0, len(index) - batch_size, batch_size), range(batch_size, len(index), batch_size)):
            ...
            _, loss_val,multilabel_loss_val = sess.run(
                [train_op, tf_loss,tf_multilabel_loss],
                feed_dict={
                    tf_video: video_batch,
                    tf_labels: sample_labels
                })
            loss_to_draw_epoch.append(loss_val)
            multilabel_loss_to_draw_epoch.append(multilabel_loss_val)
            ...
            if np.mod(iter_count, 2000) == 0:
                ...
                saver.save(sess, os.path.join(model_path,
                                              str(n_video_lstm_step) + '_lr_'+ str(start_learning_rate) + '_batch_size' + str(batch_size) +'alpha_'+ str(alpha)+ 'attribute_model'),
                           global_step=iter_count)
                if early_stopping > 80:
                    return
                early_stopping += 1
                print 'highest mAP: ',highest_mAP

            print 'idx: ', start, ' rate: ', sess.run(learning_rate)," Epoch: ", epoch, " loss: ", loss_val, ' multilabel_loss: ', multilabel_loss_val,\
                ' Elapsed time: ', str((time.time() - start_time))

        loss_to_draw.append(np.mean(loss_to_draw_epoch))
        multilabel_loss_to_draw.append(np.mean(multilabel_loss_to_draw_epoch))
