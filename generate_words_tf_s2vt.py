...
class Video_Caption_Generator():
    ...
    def build_model(self):
        ...
        caption_mask = []
        one_mask = [[1] for x in xrange(self.batch_size)]
...
##################### compute choose word probability###
        ...
        true_word_prob = tf.expand_dims(tf.convert_to_tensor([0.5],dtype=tf.float64), 0)
        ...
        with tf.variable_scope("s2vt") as scope:
            ...
            for i in range(0, self.n_caption_lstm_step):  ## Phase 2 => only generate captions
                ...
                if i == 0:
                    ...
                    generated_word = tf.stop_gradient(generated_word)
                ...
                one_mask = tf.multiply(one_mask, tf.cast(tf.not_equal(generated_word, tf.convert_to_tensor([0],tf.int32)),tf.float32))
                caption_mask.append(one_mask)

                cross_entropy = tf.nn.softmax_cross_entropy_with_logits(labels=onehot_labels, logits=logit_words)
                cross_entropy = cross_entropy * one_mask  ######### need to move caption_mask (cont_sent)one column left ##########################llj
                ...
                #for v in tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES):
                    #print(v)
        caption_mask = tf.stack(caption_mask)
