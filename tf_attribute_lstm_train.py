...
nums_label = 400
threshold = 0.5
lstm_dim = 500 ######### choose 500 or 1000
...
class Multilabel():
    def __init__(self, batch_size, loss_weight = 1, decay_value = 0.00005, dropout_rate = 0.9,
        ...
        self.attr_W = tf.Variable(tf.random_uniform([lstm_dim, self.label_dim], -0.1, 0.1),dtype=tf.float32, name='attr_W', trainable=True)
        self.attr_b = tf.Variable(tf.zeros([self.label_dim], tf.float32),dtype=tf.float32, name='attr_b')

        self.lstm = tf.contrib.rnn.BasicLSTMCell(lstm_dim, state_is_tuple=False)
        self.lstm_dropout = tf.contrib.rnn.DropoutWrapper(self.lstm, output_keep_prob=self.dropout_rate)
    ...
    def build_model(self):
        ...
        state = tf.zeros([self.batch_size, self.lstm.state_size], tf.float32)

        with tf.variable_scope("attribute_lstm") as scope:
            for i in range(0, self.n_video_lstm_step):
                if i > 0:
                    tf.get_variable_scope().reuse_variables()

                with tf.variable_scope("LSTM"):
                    output, state = self.lstm_dropout(video[:, i, :], state)

        attribute_feature = output
    ...
    def evaluate_multilabel(self, threshold):
        ...
        state = tf.zeros(tf.stack([tf.shape(video)[0], self.lstm.state_size]), tf.float32)
        ...
        with tf.variable_scope("attribute_lstm") as scope:
            for i in range(0, self.n_video_lstm_step):
                ...
                with tf.variable_scope("LSTM"):
                    output, state = self.lstm(video[:, i, :], state)
...
def train():
    ...
    optimistic_restore(sess, 'multilabel_models/batch_size8multilabel_lstm3e-3_stopgradient_dim500-64')

    loss_to_draw = []

    for epoch in xrange(65,n_epochs):
        loss_to_draw_epoch = []
        ...
        with open('./multilabel_models/evaluate_multilabel_lstm3e-3_stopgradient_dim500_val.txt','a') as f:
            scores = []
            true_positive = 0
            true_negative = 0
            false_positive = 0
        ...
        # with open('./multilabel_models/evaluate_multilabel_lstm3e-3_stopgradient_dim500_test.txt','a') as f:
        #     scores = []
        #     true_positive = 0
        #     true_negative = 0
        #     false_positive = 0
        ...
        if np.mod(epoch, 4) == 0:
            ...
            saver.save(sess, os.path.join(model_path, 'batch_size' + str(batch_size)+ 'multilabel_lstm3e-3_stopgradient_dim500'), global_step=epoch)
...
def evaluation():
    ...
    with open('./multilabel_models/evaluate_multilabel_lstm3e-1_stopgradient_dim500_test.txt','a') as f:
      #for i in xrange(40,100,4):
        #i = 60
        model_path_last = model_path + '/batch_size8multilabel_lstm3e-3_stopgradient_dim500-60' #+ str(i)
