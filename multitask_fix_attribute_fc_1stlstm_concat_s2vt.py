...
class Video_Caption_Generator():
    def __init__(self, dim_image, n_words, word_dim, lstm_dim, batch_size, n_lstm_steps, n_video_lstm_step,
        ...
        self.attr_W = tf.Variable(tf.random_uniform([conv_dim, self.label_dim], -0.1, 0.1), dtype=tf.float32,
    ...
    def build_model(self):
        ########## inception resnet v2####
        ###preprocessing###
        video_frames = tf.placeholder(tf.float32,[self.batch_size, self.n_video_lstm_step, self.dim_image])
        all_frames = tf.reshape(video_frames,[-1, self.dim_image])
        ...
        with slim.arg_scope(inception_resnet_v2_arg_scope()):
          with tf.variable_scope('InceptionResnetV2', 'InceptionResnetV2',
            ...
            with slim.arg_scope([slim.batch_norm, slim.dropout],
                ...
                ########### attribute ################
                net = slim.fully_connected(all_frames, conv_dim, activation_fn=None,scope = 'last_fc')####################################################  add  #####
                net = tf.stop_gradient(net)
                #net = slim.flatten(net)
                #net = slim.dropout(net, self.dropout_rate, is_training=True, scope='Dropout')

        video = tf.reshape(video_frames, [self.batch_size, self.n_video_lstm_step, self.dim_image])
        ########### attibute  ###############3
        attribute_video = tf.reshape(net, [self.batch_size, self.n_video_lstm_step, conv_dim])####################################################  add  #####
        #####################################################
        ...
        #####################  attribute ############
        fusion_feature = tf.concat([image_emb, attribute_video],axis=2)####################################################  add / multiply / concatenate #####

        ### add dropout####
        image_emb = fusion_feature
        ...
        concat_padding = tf.zeros([self.batch_size, self.word_dim+conv_dim], tf.float32)
        ...
        with tf.variable_scope("s2vt") as scope:
            ...
            for i in range(0, self.n_caption_lstm_step):  ## Phase 2 => only generate captions
                ...
                tf.get_variable_scope().reuse_variables()
                with tf.variable_scope("LSTM1"):
                    output1, state1 = self.lstm1_dropout(concat_padding, state1)
    ...
    def build_sampler(self):
        video_frames = tf.placeholder(tf.float32,
                                      [None, self.n_video_lstm_step, self.dim_image])
        all_frames = tf.reshape(video_frames, [-1, self.dim_image])
        ...
        with slim.arg_scope(inception_resnet_v2_arg_scope()):
            with tf.variable_scope('InceptionResnetV2', 'InceptionResnetV2',
                ...
                with slim.arg_scope([slim.batch_norm, slim.dropout],
                    ...
                    attribute_net = slim.fully_connected(all_frames, conv_dim, activation_fn=None,scope = 'last_fc')####################################################  add  #####

        video = tf.reshape(video_frames, [-1, self.n_video_lstm_step, self.dim_image])
        attribute_video = tf.reshape(attribute_net,[-1, self.n_video_lstm_step, conv_dim])####################################################  add  #####
        video_flat = tf.reshape(video, [-1, self.dim_image])
        image_emb = tf.nn.xw_plus_b(video_flat, self.encode_image_W, self.encode_image_b)
        ...
        fusion_feature = tf.concat([image_emb, attribute_video],axis=2)####################################################  add /mul/concat #####
        image_emb = fusion_feature
        ...
        concat_padding = tf.zeros(tf.stack([tf.shape(video)[0], self.word_dim+conv_dim]), tf.float32)
        ...
        with tf.variable_scope("s2vt") as scope:
            ...
            for i in range(0, self.n_caption_lstm_step):
                ...
                with tf.variable_scope("LSTM1"):
                    output1, state1 = self.lstm1(concat_padding, state1)
                with tf.variable_scope("LSTM2"):
                    output2, state2 = self.lstm2(tf.concat([output1, current_embed], 1), state2)
                logit_words = tf.nn.xw_plus_b(output2, self.embed_word_W, self.embed_word_b)
...
video_test_feature_file = '/home/llj/tensorflow_s2vt/5frame_val.txt'
...
feature_model = '/home/llj/tensorflow_s2vt/multilabel_models/batch_size8multilabel_fc3e-1_stopgradient_dim1000-34'
...
out_file = 'fix_models/fix_models_attribute_fc_1stlstm_1e-4_concat_dim1000.txt'

model_name = 'fix_models_attribute_fc_1stlstm_1e-4_concat_dim1000'
    ...
    for epoch in range(0, n_epochs):
       ...
       # train_captions, video_frames, train_vid = get_video_feature_caption_pair(video_train_sent_file, video_path,num_frame_per_video=n_video_lstm_step)
        ...
        for start, end in zip(range(0, len(index) - batch_size, batch_size), range(batch_size, len(index), batch_size)):
            ...
            print 'idx: ', start, ' rate: ', sess.run(learning_rate)," Epoch: ", epoch, " loss: ", loss_val,' Elapsed time: ', str((time.time() - start_time))
        ...
        plt_save_dir = "./fix_loss_imgs"
        plt_save_img_name = str(epoch) + '_5uniform_attibute_fc_1stlstm_cider_concat_third_val.png'
        plt.figure(1)
        ...
        plt_save_dir = "./fix_loss_imgs"
        plt_save_img_name = str(epoch) + '_5uniform_concat_third_val.png'
        plt.figure(2)
        ...
        if np.mod(epoch, 1) == 0:
            ...
            saver.save(sess, os.path.join(model_path, 'batch_size' + str(batch_size)+ '_'+ model_name), global_step=epoch)
            if early_stopping > 9:
                return
            early_stopping += 1
...
def evaluation(model_path='/home/llj/tensorflow_s2vt/fix_models/'):
    test_captions, test_features = get_video_feature_caption_pair(video_test_sent_file, video_test_feature_file)
    ...
    with open('fix_models/fix_models_attribute_fc_1stlstm_1e-4_concat_dim1000_val.txt', 'a') as f:
      for i in xrange(0,18,1):
        model_path_last = model_path + 'batch_size16_fix_models_attribute_fc_1stlstm_1e-4_concat_dim1000-' +str(i)
        saver.restore(sess,model_path_last)
        all_decoded_for_eval = {}
        test_index = list(range(len(test_captions)))
        random.shuffle(test_index)
