...
class Video_Caption_Generator():
    def __init__(self, dim_image, n_words, word_dim, lstm_dim, batch_size, n_lstm_steps, n_video_lstm_step,
                 n_caption_lstm_step, bias_init_vector=None, loss_weight = 20, decay_value = 0.00005, dropout_rate = 0.9, k_value = 7000,steps = 1):
            ...
            for i in range(0, self.n_caption_lstm_step):  ## Phase 2 => only generate captions
                if i == 0:
                    ...
                    with tf.device("/cpu:0"):
                        ...
                        current_embed1 = tf.nn.embedding_lookup(self.Wemb, generated_word)  ##without cpu    ### i-1 correspond to the previous word
                        current_embed = tf.divide(tf.add(current_embed, current_embed1),2)
                #### add dropout##
    ...
    for epoch in range(0, n_epochs):
        ...
        for start, end in zip(range(0, len(index) - batch_size, batch_size), range(batch_size, len(index), batch_size)):
            ...
            print 'idx: ', start, ' rate: ', sess.run(learning_rate)," Epoch: ", epoch, " loss: ", loss_val,\
                ' Elapsed time: ', str((time.time() - start_time)) , 'steps: ',model.steps
            loss_fd.write('epoch ' + str(epoch) + ' loss ' + str(loss_val) + '\n')

        # draw loss curve every epoch
        loss_to_draw.append(np.mean(loss_to_draw_epoch))
        plt_save_dir = "./generated_words_loss_imgs"
        ...
        if np.mod(epoch, 1) == 0:
            ...
            saver.save(sess, os.path.join(model_path, 'generated-model'), global_step=epoch)
...
def test(model_path='/home/llj/tensorflow_s2vt/generated_words_models/'):
    ...
    for i in xrange(12):
        model_path_last = model_path + 'generated-' + 'model-' + str(i) #+ '.data-00000-of-00001'
        out_file = 'generated_vocab51915_batchsize_8_ep'+ str(i) + '.txt'
