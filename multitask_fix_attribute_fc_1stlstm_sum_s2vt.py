...
class Video_Caption_Generator():
    ...
    def build_model(self):
        ...
        #####################  attribute ############
        fusion_feature = image_emb + attribute_video####################################################  add / multiply / concatenate #####
        ...
        concat_padding = tf.zeros([self.batch_size, self.word_dim], tf.float32)
    ...
    def build_sampler(self):
        ...
        fusion_feature = image_emb + attribute_video####################################################  add /mul/concat #####
        image_emb = fusion_feature
        ...
        concat_padding = tf.zeros(tf.stack([tf.shape(video)[0], self.word_dim]), tf.float32)
...
out_file = 'fix_models/fix_models_attribute_fc_1stlstm_1e-4_sum.txt'

model_name = 'fix_models_attribute_fc_2ndlstm_1e-4_dim1000_sum'

cider_img = 'attribute_fc_2ndlstm_1e-4_sum_dim1000_cider.png'

loss_img = 'attribute_fc_2ndlstm_1e-4_sum_dim1000_loss.png'
...
######################################### caption evaluation ##########################
    ...
    for epoch in range(0, n_epochs):
       ...
       # train_captions, video_frames, train_vid = get_video_feature_caption_pair(video_train_sent_file, video_path,num_frame_per_video=n_video_lstm_step)
        ...
        plt_save_img_name = str(epoch) + cider_img
        plt.figure(1)
        ...
        plt_save_dir = "./fix_loss_imgs"
        plt_save_img_name = str(epoch) + loss_img
        plt.figure(2)
...
def evaluation(model_path='/home/llj/tensorflow_s2vt/fix_models/'):
    ...
    with open('fix_models/batch_size16_fix_models_attribute_fc_1stlstm_1e-4_sum_test.txt', 'a') as f:
      for i in xrange(1,9,1):
        model_path_last = model_path + 'batch_size16_fix_models_attribute_fc_1stlstm_1e-4_sum-' +str(i)
        saver.restore(sess,model_path_last)
        all_decoded_for_eval = {}
        test_index = list(range(len(test_captions)))
        random.shuffle(test_index)
