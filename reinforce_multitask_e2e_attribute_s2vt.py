    ...
    for epoch in range(0, n_epochs):
        ...
        for start, end in zip(range(0, len(index)-batch_size, batch_size), range(batch_size, len(index), batch_size)):
           ...
           # r = [evaluate_captions([k],[v]) for k,v in zip(ref_decoded, multi_decoded)]
            ...
            feed_dict = {loss_masks: mask, loss_captions: samples, loss_features: video_batch, rewards: r, base_line: b, model_features: video_batch, model_captions: captions_ind, model_caption_masks: captions_mask}
