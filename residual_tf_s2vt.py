...
def train():  ###### move caption (input_sentence) one column left and also need to move caption_mask (cont_sent)one column left ########################################################llj
    ...
    #saver.restore(sess,'res_models/batch_size16residual_rgb_val-11')
   ...
   # summary_op = tf.summary.merge_all()
    ...
    for epoch in range(0, n_epochs):
        ...
        with open('res_models/train_residual_scores_val.txt', 'a') as f:
        ...
        plt_save_dir = "./res_loss_imgs_val"
        plt_save_img_name = str(epoch) + '_residual.png'
        ...
        if np.mod(epoch, 1) == 0:
            ...
            saver.save(sess, os.path.join(model_path, 'batch_size' + str(batch_size)+ 'residual_rgb_val'), global_step=epoch)
...
def evaluation(model_path='/home/llj/tensorflow_s2vt/res_models/'):
    test_captions, test_features = get_video_feature_caption_pair(video_test_sent_file, video_test_feature_file)
    ...
    model_path_last = model_path + 'batch_size16residual_rgb_val-4'
    ...
    with open('res_models/train_noval_resmodel_scores_on_test.txt', 'a') as f:
        all_decoded_for_eval = {}
        test_index = list(range(len(test_captions)))
        random.shuffle(test_index)
        ref_decoded = {}
